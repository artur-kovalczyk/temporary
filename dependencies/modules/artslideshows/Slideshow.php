...
class Slideshow extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'artslideshows_slides',
		'primary' => 'id_artslideshow',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'artslideshows` (`id_shop`, `id_artslideshow`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'artslideshows`
			WHERE `id_artslideshow` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_artslideshow` = hs.`id_artslideshow` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_artslideshow` as id_slide
			FROM `'._DB_PREFIX_.'artslideshows_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'artslideshows` hs ON (hss.`id_artslideshow` = hs.`id_artslideshow`)
